...
export function validatePasswordStrength(password: string): void {
    ...
    if (password.length < PASSWORD_REQUIREMENTS.minLength) {
        throw new ValidationError(`Password must be at least ${PASSWORD_REQUIREMENTS.minLength} characters long`);
    }

    if (password.length > PASSWORD_REQUIREMENTS.maxLength) {
        throw new ValidationError(`Password must be no more than ${PASSWORD_REQUIREMENTS.maxLength} characters long`);
    }
